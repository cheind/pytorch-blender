...
In this paper we propose a principled way of generating artificial training data from a simulator engine that is directly looped into the training procedure, thereby avoiding early data commitment. A bi-directional communication channel enables adaptive random data generation. We demonstrate the usefulness of our approach to the detection of robot poses in color images using non-realistic computer images.

% Note, annotation of robots using experts is hard! Compare hand labelled images from Paul,Christoph,Sebastian -> better use a simulator!
