...
\subsection{Artificial Data for Training}
    ...
    In the context of robot joint localization, we think of training image generation as a process that includes random and deterministic calculations. Hence, we describe the image generation process in terms of a probabilistic graphical model that describes relationships of hidden and observed variables in terms of prior and conditional probability distributions. The observed variables in our model correspond to pixels in the generated images $\mathbf{I}$. The hidden variables describe the model behind these images. Hidden variables include robot joint angles and the virtual camera position. Robot joint angles are represented as vector $\mathbf{J}$ of - in our case - six joint angles. These joint angles determine the actual locations of robot joints $\mathbf{R}$. Virtual camera position $\mathbf{C}$ describes varying camera positions in the scene. We parameterize $\mathbf{C}$ by angles and distances. The virtual camera position $\mathbf{C}$ can easily be converted to a roto-translation matrix $\mathbf{T}$ describing the virtual camera position in the scene. Given the robot joint locations $\mathbf{R}$ and camera location $\mathbf{T}$, we can describe the corresponding 3D scene in Blender. The Blender rendering engine is then used to create the final images $\mathbf{X}$. Given $\mathbf{R}$ and $\mathbf{T}$, it is easily possible to calculate the joint positions $\mathbf{R}$ in the virtual camera images. We denote these projected 2d points $\mathbf{P}$.
    
    Figure \ref{fig:probModel} illustrates this as a graphical model. Note that the arrows in the figure in our case correspond to deterministic calculations. Therefore, the only actual probability distributions involved relate to $\mathbf{J}$ and $\mathbf{C}$. Intrinsic camera parameters are assumed to be fixed and are not explicitly shown in the graphical model.

    \begin{figure}[htbp]
        \centerline{\includegraphics[width=0.4\columnwidth]{figures/probModel/probModel.pdf}}
        \caption{\label{fig:probModel} The image rendering process as probabilistic model.}
    \end{figure}
    
    To be more explicit, we model $C$ as a vector of variables: $\mathbf{C} = (\theta, \phi, d, \mathbf{c}, \mathbf{r})$. $\theta$ and $\phi$ are elevation and azimuth angles for the vector pointing from world coordinate origin to the camera position. $d$ is the distance of the camera from the origin. The position at which the virtual camera is looking is chosen close to the origin. A fixed point is specified and a random deviation from that point with a maximum distance of $r$ is sampled.
    
    TODO: Values for individual paramters... (?)

    The problem at hand is to derive the hidden joint locations $\mathbf{P}$ in the camera images. Based on the probabilistic model it is not possible to invert calculations. Although we control the process of image generation, we have no possibility to directly infer hidden parameters given the observed images.
