...
In this paper we propose method of generating artificial training for robot pose detection  using multiple simulator engines that are directly looped into the training procedure. A feedback communication channel enables adaptiveness of random data generation during training. Our approach avoids early data commitment and enables interactive training data generation. We note that, while our description below is geared towards robot pose estimation from color images, the principles of our method hold for arbitrary applications.
...
TODO describe artificial data generation and prob model -> szamba

\subsection{Joint Localization Model}
...
Messages between simulation and training blocks are exchanged via ZMQ \cite{zeromq_guide} using TCP/IP transport layer protocol. 
