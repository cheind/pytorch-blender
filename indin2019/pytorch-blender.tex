...
\begin{abstract}
Impressive results have been achieved in computer vision via deep learning methods over the last years. However, for many highly specialized industrial applications these methods cannot easily be applied due to the lack of large amounts of training data. In this paper we propose a software framework that provides artificial image data via simulation. This approach opens the door for machine learning even when it is hard or impossible to acquire real (labelled) images. Our system supports parallelized data generation and direct feedback from neural network training to adaptively put focus on critical parameter ranges. We demonstrate that the model trained on artificial robot images is able to generalize to real world images. 
\end{abstract}
...
%\begin{figure}[htbp]
%\centerline{\includegraphics[width=\columnwidth]{figures/examplesUR10/renderedImages.jpg}}
