...
class Supershape:
    ...
    def __init__(self, Unum=50, Vnum=50, smooth=True, name='supershape'):
        ...
        faces = self._faces()        
        verts = self._vertices([1.2, 0.06, 2.78, 0.5, 0.5, 1.5])
        edges = []

        self.mesh = bpy.data.meshes.new('supershape')
        self.mesh.from_pydata(verts,edges,faces)
        self.mesh.update(calc_edges=True)
        self.obj = bpy.data.objects.new('supershape', self.mesh)
        ...
        self.update([1.2, 0.06, 2.78, 0.5, 0.5, 1.5])
    ...
    def _vertices(self, params):
        # See https://en.wikipedia.org/wiki/Superformula
        m,a,b,n1,n2,n3 = params
