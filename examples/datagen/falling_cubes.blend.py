...
def main():
    ...
    for idx, c in enumerate(cubes):
        ...
        mat.diffuse_color = np.concatenate((np.random.random(size=3), [1.0]))
        c.data.materials.append(mat)
        c.active_material = mat

    def pre_anim():
        xyz = np.random.uniform((-3, -3, 6), (3, 3, 12.0), size=(len(cubes), 3))
    ...
    def post_frame(anim, off, pub, cam):
        pub.publish(
            image=off.render(), xy=cam.object_to_pixel(*cubes), frameid=anim.frameid
        )
