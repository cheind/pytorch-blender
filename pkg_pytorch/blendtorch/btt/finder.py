...
def discover_blender(additional_blender_paths=None):
    ...
    p = subprocess.Popen(
        f'"{bpath}" --background --python-use-system-env --python-exit-code 255 --python {fp.name}',
        shell=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        env=my_env,
    ...
    if errcode != 0:
        logger.warning(
            "Failed to run minimal Blender script; ensure Python requirements are installed."
        )
        return None
